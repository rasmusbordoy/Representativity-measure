...
head(df)
df[,sum(Response)/.N,by=c("Male","Female")][,length(c("Male","Female"))+1]
...
for( i in 1:(16-1)){
  if(i==1){
    B <- expand.grid(A[1],A[-1])
    A <- A[-1]
  }else{
    B <- rbind(expand.grid(A[1],A[-1]),B)
    A <- A[-1]
  }
...
setDT(r)
rbind(head(A[order(A$`Response %`)]),
r,
tail(A[order(A$`Response %`)]))
...
if(length(sel_cov)>0 & class(m)[1] == "cv.glmnet")
{
...
if(class(m)[1] == "cv.glmnet"){ 
  df_R <- cbind(df,P=predict(m,as.matrix(df[,-c("COV","Response")]),type="response",s=m$lambda.1se))
  colnames(df_R)[ncol(df_R)] <- "P"
  beta <- coef(m,s=m$lambda.1se)
